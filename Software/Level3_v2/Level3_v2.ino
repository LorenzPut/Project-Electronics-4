...
void Level3()
{
  LDRvalueRight = MeasureLDR(LDRpinRight);
  LDRvalueLeft = MeasureLDR(LDRpinLeft);
  Printonscreen("");
  
    if(LDRvalueRight < 150 && LDRvalueLeft < 150)
    {    
      
      if((LDRvalueRight + 10) > LDRvalueLeft)
      {
        m.TurnRight(0);
      }
      else if((LDRvalueLeft +10) > LDRvalueRight)
      {
        m.TurnRight(0);
      }
...
void Printonscreen(String message)
{
  ...
  Serial.print("LDRwaarde links : ");
  Serial.println(LDRvalueLeft);
  
  Serial.print("LDRwaarde rechts : ");
  Serial.println(LDRvalueRight);
}
